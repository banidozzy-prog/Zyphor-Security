...
function getGuildConfig(guildId) {
    if (!db.has(guildId)) {
        db.set(guildId, {
            cargo_suporte: null,
            cargo_admin: null, // Cargo que será definido no painel
...
// Controle de Cooldown para o !not (4 usos a cada 2 hours por servidor)
...
client.once('ready', async () => {
    ...
    try {
        ...
        console.log('🚀 Sistema de Configuração Protegido Online!');
...
client.on('interactionCreate', async (i) => {
    ...
    // TRAVA DE SEGURANÇA: Só quem tem o CARGO ADMIN configurado ou tem a permissão Administrator nativa do Discord pode usar!
    ...
    // Proteção extra para as interações do painel de configuração
    if (i.isStringSelectMenu() || i.isButton() || i.isChannelSelectMenu() || i.isRoleSelectMenu() || i.isModalSubmit()) {
        const customIdsConfig = ['menu_mestre_config', 'sel_destino_categoria', 'sel_destino_canal_threads', 'sel_canais_welcome', 'sel_logs_principais', 'sel_logs_seguranca', 'sel_cargo_admin', 'sel_cargo_suporte', 'btn_msg_welcome', 'modal_welcome_text', 'add_ticket_opcao', 'modal_add_ticket', 'limpar_ticket_opcoes', 'enviar_painel_membros'];
...
client.on('guildMemberAdd', async (m) => {
    ...
    if (c) {
        ...
        const embedWelcome = new EmbedBuilder()
            ...
            .setImage(fotoMembro) 
            .setColor('#4f46e5')
            .setFooter({ text: `Agora somos ${m.guild.memberCount} membros!` })
            .setTimestamp();
