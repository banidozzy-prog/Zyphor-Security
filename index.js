...
client.on('interactionCreate', async (i) => {
    ...
    // Resposta padrão para evitar erro de timeout
    try {
        if (i.commandName === 'painel') {
            await i.reply({ content: '⚙️ **Painel de Segurança Zyphor V3**\nO sistema de proteção Anti-Burla e Anti-Scam está **ATIVO**.', ephemeral: true });
        } 
        else if (i.commandName === 'addpalavra') {
            db.palavras.push(i.options.getString('palavra').toLowerCase());
            save();
            await i.reply({ content: `✅ Palavra "${i.options.getString('palavra')}" adicionada.`, ephemeral: true });
        } 
        else if (i.commandName === 'remover') {
            const p = i.options.getString('palavra').toLowerCase();
            db.palavras = db.palavras.filter(word => word !== p);
            save();
            await i.reply({ content: `✅ Palavra "${p}" removida.`, ephemeral: true });
        } 
        else if (i.commandName === 'verpalavras') {
            const lista = db.palavras.length > 0 ? db.palavras.join(', ') : 'Nenhuma.';
            await i.reply({ content: `📜 **Palavras proibidas:** ${lista}`, ephemeral: true });
        } 
        else if (i.commandName === 'setlog') {
            db.logs[i.options.getString('tipo')] = i.options.getChannel('canal').id;
            save();
            await i.reply({ content: `✅ Log de ${i.options.getString('tipo')} configurado!`, ephemeral: true });
        }
    } catch (e) {
        console.error(e);
    }
